...
% PPAR menu options
PPAR.N  = '100';                                                            % "Number of panel nodes"
...
[xFoilResults,success] = XFOIL(NACA,PPAR,AoA,flagAirfoil,Re);                  % Get XFOIL results for prescribed airfoil
...
BLx     = xFoilResults.BLx;
BLy     = xFoilResults.BLy;
Dstar   = xFoilResults.Dstar;
theta   = xFoilResults.theta;
Cf      = xFoilResults.Cf;
H       = xFoilResults.H;
...
while any(error > 0.000001)
    ...
    for i = 1:1:numPan                                                          % Loop over all i panels (rows)
        b(i) = 2*pi*vtran(i)-Vinf*2*pi*cos(beta(i));                            % Compute RHS array
    end
    ...
    for i = 1:1:numPan
        ...
        term5 = Vinf*cos(beta(i));
        term6 = (1/(2*pi))*sum(lambda.*I(i,:)');
        term7 = lambda(i)/2;
        term8 = -(gamma/(2*pi))*sum(K(i,:));

        Vt(i) = term1 + term2 + term3 + term4;                                  % Compute tangential velocity on panel i
        Vn(i) = term5 + term6 + term7 + term8;                                  % Compute tangential velocity on panel i

        Cp(i) = 1-(Vt(i)^2+Vn(i)^2)/Vinf^2;                                     % Compute pressure coefficient on panel i
    end
    %% Determine Stagnation Point
    i_s = STAGNATION(numPan,Vt);
    fprintf("i_s : %d \n", i_s);
    
    %% Thwaites Methods for Laminary Boundary Layer
    [sep_u ,sep_l,theta_total,disp_total,H_total,Cf_total,trans_u,trans_l,vtran] = THWAITES(Vt,XC,i_s,nu);
    
    fprintf("upper transition : %d \n", XC(trans_u));
    fprintf("lower transition : %d \n", XC(trans_l));
    %% M.R. Head Entrainment Method for Turbulent Boundary Layer
    [theta_total,H_total,Cf_total,disp_total,vtran,sep_u,sep_l] = TURBULENT(Vt,XC,i_s,nu,trans_u,trans_l,theta_total,H_total,Cf_total,disp_total);

    %% Calculate Error    
    if iter == 1
        disp_t_old = disp_total;
        iter = iter + 1;
    else
        error = abs(disp_total-disp_t_old)/disp_total;
        disp_t_old = disp_total;
        iter = iter + 1;
    end
end
...
%CN = -Cp.*S.*sin(beta)+Cf_thwaites.*S.*sin(beta);                           % Normal force coefficient []
%CA = -Cp.*S.*cos(beta)+Cf_thwaites.*S.*cos(beta);                           % Axial force coefficient []
Cf_total(sep_u:end) = 0
CN = -Cp.*S.*sin(beta)-abs(Cf_total.*S.*cos(beta));
CA = -Cp.*S.*cos(beta)+abs(Cf_total.*S.*sin(beta));

% Compute lift and moment coefficients
CL = sum(CN.*cosd(AoA)) - sum(CA.*sind(AoA));                               % Decompose axial and normal to lift coefficient []
CD = sum(CN.*sind(AoA)) + sum(CA.*cosd(AoA));
...
fprintf('\tSPVP : %f\n',CD);                                             % From this SPVP code
fprintf('\tXFOIL: %f\n',xFoilCD);                                        % From XFOIL program
...
if (flagPlot(4) == 1)
    ...
    pVu = plot(XC(midIndS+1:end),Cp(midIndS+1:end),'ks-',...                 % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    %psep = plot([XC(sep_u) XC(sep_l)],[Cp(sep_u) Cp(sep_l)],'m+','LineWidth',2);
    legend([pXu,pXl,pVu,pVl],...                                            % Show legend
           {'XFOIL Upper','XFOIL Lower','Present Upper','Present Lower'});
...
% FIGURE: Pressure coefficient contour
if (flagPlot(7) == 1)
    figure(7);                                                              % Create figure
    cla; hold on; grid on;                                                  % Get ready for plotting
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    pdisp_u=plot(XC(i_s:numPan),YC(i_s:numPan)+disp_total(i_s:numPan));   % Plot Displacement Thickness
    pdisp_l=plot(XC(1:i_s-1),YC(1:i_s-1)-disp_total(1:i_s-1));            % Plot Displacement Thickness
    ptrans = plot([XC(trans_u) XC(trans_l)],[YC(trans_u) YC(trans_l)],'ro');
    %psep = plot([XC(sep_u) XC(sep_l)],[YC(sep_u) YC(sep_l)],'r+');
    pfoil = plot(XB,YB,'k--');                                                        % Plot airfoil as black polygon
    legend([pfoil pdisp_u pdisp_l ptrans ],{'Airfoil','Upper Boundary Layer','Lower Boundary Layer','Transition Point'})
    xlabel('X Units');                                                      % Set X-label
    ylabel('Y Units');                                                      % Set Y-label
    xlim([-0.1 1.1]);                                                            % Set X-axis limits
    axis equal;                                                             % Set axes equal
    ylim([-0.1 0.1]);                                                            % Set Y-axis limits
    zoom reset;                                                             % Reset zoom

    figure(8);                                                              % Create figure
    cla; hold on; grid on;                                                  % Get ready for plotting
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    midIndX = floor(length(BLx)/2);                                         % Airfoil middle index for XFOIL data
    midIndS = floor(length(Dstar)/2);                                       % Airfoil middle index for SPM data
    pXu = plot(BLx(1:midIndX),Dstar(1:midIndX),'b-','LineWidth',2);         % Plot Cp for upper surface of airfoil from XFOIL
    pXl = plot(BLx(midIndX+1:end),Dstar(midIndX+1:end),'r-',...             % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(1:i_s-1),disp_total(1:i_s-1),'ks-','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(i_s:numPan),disp_total(i_s:numPan),'ks-',...                         % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    %psep = plot([XC(sep_u) XC(sep_l)],[disp_total(sep_u) disp_total(sep_l)],'m+','LineWidth',2);
    legend([pXu,pXl,pVu,pVl],...                                            % Show legend
           {'XFOIL Upper','XFOIL Lower','Present Upper','Present Lower'});
    xlabel('X Coordinate');                                                 % Set X-label
    ylabel('Displacement Thickness');                                       % Set Y-label
    xlim([0 1]);                                                            % Set X-axis limits
    ylim('auto');                                                           % Set Y-axis limits to auto
    title(['Airfoil: ' xFoilResults.afName])                                % Title
    zoom reset;                                                             % Reset zoom

    figure(9);                                                              % Create figure
    cla; hold on; grid on;                                                  % Get ready for plotting
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    midIndX = floor(length(BLx)/2);                                         % Airfoil middle index for XFOIL data
    midIndS = floor(length(theta)/2);                                       % Airfoil middle index for SPM data
    pXu = plot(BLx(1:midIndX),theta(1:midIndX),'b-','LineWidth',2);         % Plot Cp for upper surface of airfoil from XFOIL
    pXl = plot(BLx(midIndX+1:end),theta(midIndX+1:end),'r-',...             % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(1:i_s-1),theta_total(1:i_s-1),'ks-','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(i_s:numPan),theta_total(i_s:numPan),'ks-',...                         % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    %psep = plot([XC(sep_u) XC(sep_l)],[theta_total(sep_u) theta_total(sep_l)],'m+','LineWidth',2);
    legend([pXu,pXl,pVu,pVl],...                                            % Show legend
           {'XFOIL Upper','XFOIL Lower','Present Upper','Present Lower'});
    xlabel('X Coordinate');                                                 % Set X-label
    ylabel('Momentum Thickness');                                       % Set Y-label
    xlim([0 1]);                                                            % Set X-axis limits
    ylim('auto');                                                           % Set Y-axis limits to auto
    title(['Airfoil: ' xFoilResults.afName])                                % Title
    zoom reset;              

    figure(10);                                                              % Create figure
    cla; hold on; grid on;                                                  % Get ready for plotting
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    midIndX = floor(length(BLx)/2);                                         % Airfoil middle index for XFOIL data
    midIndS = floor(length(H)/2);                                       % Airfoil middle index for SPM data
    pXu = plot(BLx(1:midIndX),H(1:midIndX),'b-','LineWidth',2);         % Plot Cp for upper surface of airfoil from XFOIL
    pXl = plot(BLx(midIndX+1:end),H(midIndX+1:end),'r-',...             % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(1:i_s-1),H_total(1:i_s-1),'ks-','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(i_s:end),H_total(i_s:end),'ks-',...                         % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    %psep = plot([XC(sep_u) XC(sep_l)],[YC(sep_u) YC(sep_l)],'m+');
    legend([pXu,pXl,pVu,pVl],...                                            % Show legend
           {'XFOIL Upper','XFOIL Lower','Present Upper','Present Lower'});
    xlabel('X Coordinate');                                                 % Set X-label
    ylabel('Shape Factor, H');                                       % Set Y-label
    xlim([0 1]);                                                            % Set X-axis limits
    ylim('auto');                                                           % Set Y-axis limits to auto
    title(['Airfoil: ' xFoilResults.afName])                                % Title
    zoom reset;         

    figure(11);                                                              % Create figure
    cla; hold on; grid on;                                                  % Get ready for plotting
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    midIndX = floor(length(BLx)/2);                                         % Airfoil middle index for XFOIL data
    midIndS = floor(length(Cf)/2);                                       % Airfoil middle index for SPM data
    pXu = plot(BLx(1:midIndX),Cf(1:midIndX),'b-','LineWidth',2);         % Plot Cp for upper surface of airfoil from XFOIL
    pXl = plot(BLx(midIndX+1:end),Cf(midIndX+1:end),'r-',...             % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(1:i_s-1),Cf_total(1:i_s-1),'ks-','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(i_s:end),Cf_total(i_s:end),'ks-',...                         % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    %psep = plot([XC(sep_u) XC(sep_l)],[Cf_total(sep_u) Cf_total(sep_l)],'m+','LineWidth',2);
    legend([pXu,pXl,pVu,pVl],...                                           % Show legend
           {'XFOIL Upper','XFOIL Lower','Present Upper','Present Lower'});
    xlabel('X Coordinate');                                                 % Set X-label
    ylabel('Friction Coefficient, Cf');                                       % Set Y-label
    xlim([0 1]);                                                            % Set X-axis limits
    ylim([-0.01 0.03]);                                                           % Set Y-axis limits to auto
    title(['Airfoil: ' xFoilResults.afName])                                % Title
    zoom reset;     
end