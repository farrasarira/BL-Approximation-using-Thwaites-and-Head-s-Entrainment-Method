function [xFoilResults,success] = XFOIL(NACA,PPAR,AoA,flagAirfoil,Re)
...
saveFlnmBL = ['Save_' airfoilName '_BL.txt'];                               % Airfoil Boundary Layer save-to file
...
fprintf(fid,['DUMP ' saveFlnmBL '\n']);                                      % Write Boundary Layer File
fprintf(fid,'PWRT\n');                                                      % Save the polar data
...
% Load and read boundary layer file
fidBL = fopen(saveFlnmBL);                                                  % Open boundary layer data file for reading

dataBuffer = textscan(fidBL,'%f %f %f %f %f %f %f %f','HeaderLines',1);     % Read data from file
                                 
fclose(fidBL);                                                              % Close the file
delete(saveFlnmBL);                                                         % Delete the file

% Extract polar data from buffer into function solution variable
xFoilResults.BLx = dataBuffer{2};                                     
xFoilResults.BLy = dataBuffer{3};                                     
xFoilResults.Dstar = dataBuffer{5}; 
xFoilResults.theta = dataBuffer{6}; 
xFoilResults.Cf = dataBuffer{7};
xFoilResults.H = dataBuffer{8}; 